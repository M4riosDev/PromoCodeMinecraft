...
public class NetworkHandler {
        ...
        public static void handle(RedeemCodePacket pkt, Supplier<NetworkEvent.Context> ctx) {
            ctx.get().enqueueWork(() -> {
                ...
                switch (outcome.result) {
                    case SUCCESS:
                        msg = "§aCode redeemed! Enjoy your rewards!"; success = true;
                ...
                CHANNEL.sendTo(new RedeemResultPacket(success, msg), player.connection.connection, net.minecraftforge.fml.network.NetworkDirection.PLAY_TO_CLIENT);
                CHANNEL.sendTo(
    ...
    public static class RedeemResultPacket {
        ...
        public RedeemResultPacket(boolean success, String message) { this.success = success; this.message = message; }
        public static void encode(RedeemResultPacket p, PacketBuffer b) { b.writeBoolean(p.success); b.writeUtf(p.message, 256); }
        public static RedeemResultPacket decode(PacketBuffer b) { return new RedeemResultPacket(b.readBoolean(), b.readUtf(256)); }
        public static void handle(RedeemResultPacket pkt, Supplier<NetworkEvent.Context> ctx) {
            ctx.get().enqueueWork(() -> PromoCodeScreen.handleResult(pkt.success, pkt.message));
            ctx.get().setPacketHandled(true);
        }
    }
    ...
    public static class OpenGuiPacket {
        ...
        public static void handle(OpenGuiPacket pkt, Supplier<NetworkEvent.Context> ctx) {
            ctx.get().enqueueWork(() -> Minecraft.getInstance().setScreen(new PromoCodeScreen()));
            ctx.get().setPacketHandled(true);
        }
    }
    ...
    public static class PromoStatsPacket {
        ...
        public static void handle(PromoStatsPacket pkt, Supplier<NetworkEvent.Context> ctx) {
            ctx.get().enqueueWork(() -> PromoCodeScreen.handleStats(pkt.redeemedTotal, pkt.maxCapacity, pkt.redeemedPlayers));
            ctx.get().setPacketHandled(true);
        }
    }
}
